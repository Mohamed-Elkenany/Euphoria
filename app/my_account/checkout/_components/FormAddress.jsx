...
import { createAddress } from "@/app/_data/actions/auth-actions";
import SubmitButton from "../../(userInfo)/delivery_address/SubmitButton";
const INITIAL_STATE = {
  data: null,
};
function FormAddress() {
  ...
  const [formState, actionForm] = useFormState(createAddress, INITIAL_STATE);
  ...
  useEffect(() => {
    if (formState?.defaultAddress) {
      console.log(true);
    } else if (formState?.address) {
      localStorage.setItem("address", JSON.stringify(formState.address));
    }
  }, [formState]);
  useEffect(() => {
  ...
  return (
    <div className="border-b-2 border-colorGrayFour pb-4">
      {formState?.error && (
        ...
        <div>
          <div className="flex items-center gap-x-2 my-3">
            ...
            <label htmlFor="information">
              <h1 className="text-colorGrayOne text-sm font-medium">
